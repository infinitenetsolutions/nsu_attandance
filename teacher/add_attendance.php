      ...
      <div class="row">
        ...
        <div class="col-md-6" align="right">
        	<select  class="selectpicker from-control" onchange="changeGrade()" id="changeGradeId" data-show-subtext="true" data-live-search="true" >
            	...
            	?>
        	            <option value="<?php echo $row["grade_id"]; ?>" <?php if($row["grade_id"] == $_GET["grade"]){ echo "disabled selected"; } ?> ><?php echo $row["grade_name"]; ?></option>
        	    <?php } ?>
              ...
              <table class="table table-striped table-bordered">
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <input type="radio" name="attendance_status<?php echo $student["student_id"]; ?>" <?php if(!isset($_GET["option"]) || $_GET["option"] == "present"){ echo "checked"; } ?> value="Present" />
                    </td>
                    <td>
                      <input type="radio" name="attendance_status<?php echo $student["student_id"]; ?>" <?php if(isset($_GET["option"]) && $_GET["option"] == "absent"){ echo "checked"; } ?> value="Absent" />
                    </td>
                  </tr>
                <?php
                }
  ...
  <div class="modal-dialog">
    <div class="modal-content">
     ...
     <form method="POST" action="export-excel.php">
          ...
          <!-- Modal footer -->
          <div class="modal-footer">
            <input type="hidden" name="withGrade" value="<?php if(!isset($_GET["grade"])) echo "all"; else echo $_GET["grade"]; ?>" />
...
$(document).ready(function(){
  ...
  var dataTable = $('#attendance_table').DataTable({
    ...
    "order":[],
    "ajax":{
      url:"add_attendance_action.php<?php if(empty($_GET["grade"])){ echo ""; } else if($_GET["grade"]=="all"){ echo ""; } else{ echo "?grade=".$_GET["grade"]; } ?>",
      method:"POST",
      data:{action:"fetch"}
    }
  });
